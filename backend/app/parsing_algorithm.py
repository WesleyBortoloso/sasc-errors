def bottom_up_algorithm(action_table, goto_table, input):
    ...
    while run == True:
        ...
        if action_movement[0][:8] == "REDUZIR":
            ...
            unexpected_symbol = action[1]
            expected_symbols = []

            for symbol, actions in action_table.items():
                if current_state in actions and actions[current_state] != "ERRO!":
                    expected_symbols.append(symbol)

            step_by_step.append(
                f"Erro sintático: símbolo '{unexpected_symbol}' inesperado no estado {current_state}."
            )
            step_by_step_detailed.append([
                f"O símbolo '{unexpected_symbol}' não era esperado neste ponto da análise.",
                f"Estado atual: {current_state}",
                f"Esperado: {', '.join(expected_symbols) if expected_symbols else 'nenhum símbolo conhecido'}",
                "Verifique a gramática e a sequência da entrada.",
            ])
            ...
            detailed_steps.append(
                {
                    ...
                    "input": input_tape.copy(),
                    "pointer": pointer,
                    "stepMarker": [unexpected_symbol, current_state],
                    "error": {
